...
open class ListSelection<E>(
    ...
    @JvmSynthetic
    override fun accept(listChanges: ListEvent<E>) {
    ...
    open fun invertSelection() {
        ...
        while (i.hasNext()) {
            val color = i.next()
            val value = internalSource[i.getIndex()]
    ...
    open fun isSelected(sourceIndex: Int): Boolean {
        if (sourceIndex < 0 || sourceIndex >= internalSource.size) {
            return false
        }
    ...
    open fun deselect(
        ...
        if (start == -1 || adjustedEnd == -1) {
            ...
            if (selectedIndex >= start && selectedIndex <= adjustedEnd) {
                deselectAll()
            }
            return
    ...
    open fun select(values: Collection<E>): Boolean {
        ...
        val indicesToSelectAsInts = IntArray(indicesToSelect.size)
        var arrayIndex = 0
        for (selectIndex in indicesToSelect) {
    ...
    private fun setSubRangeOfRange(
        ...
        for (i in minUnionIndex..maxUnionIndex) {
            ...
            val inChangeRange = i >= minChangeIndex && i <= maxChangeIndex
