...
internal class BarcodeIterator private constructor(
    ...
    override fun hasNext(): Boolean = getIndex() != barcode.size() - 1

    fun hasNextBlack(): Boolean = getIndex() < barcode.treeSize() - 1 && currentNode != null
    ...
    override fun next(): Any {
        ...
        val node = currentNode
        if (node == null) {
            return if (getIndex() < barcode.size()) {
                Barcode.WHITE
            } else {
    ...
    fun nextWhite(): Any {
        ...
        if (node == null) {
            ...
            if (getIndex() < barcode.treeSize() && getIndex() + node.rootSize >= barcode.treeSize()) {
    ...
    override fun remove() {
        if (localIndex == -1) {
            ...
            val node = currentNode!!
            if (localIndex == 0 && node.whiteSpace == 1 && getIndex() != 0) {
                findPreviousNode()
            ...
            node.removeBaseCase(getIndex(), localIndex)
            localIndex--
        }
    }
    ...
    fun setWhite(): Int {
        if (localIndex == -1) {
            ...
            if (node.rootSize == 1) {
                findNextNode()
                node.setWhite(getIndex(), localIndex, 1)
            ...
            if (node.whiteSpace + 1 == barcode.treeSize()) {
                ...
                whiteSoFar -= currentNode!!.whiteSpace
                localIndex += currentNode!!.whiteSpace + currentNode!!.rootSize
                node.setWhite(getIndex(), currentLocalIndex, 1)
                if (barcode.getRootNode() != null) barcode.treeSizeChanged()
                if (currentNode!!.whiteSpace == 0 && currentNode!!.rootSize == 0) currentNode = node
    ...
    fun setBlack(): Int {
        if (localIndex == -1) {
            ...
            val node = currentNode!!
            if (getIndex() == 0) {
                node.setBlack(getIndex(), localIndex, 1)
                if (barcode.getRootNode() != null) barcode.treeSizeChanged()
            } else {
                findPreviousNode()
                val currentLocalIndex = localIndex
                ...
                whiteSoFar -= currentNode!!.whiteSpace
                localIndex += currentNode!!.whiteSpace + currentNode!!.rootSize
                node.setBlack(getIndex(), currentLocalIndex, 1)
                if (barcode.getRootNode() != null) barcode.treeSizeChanged()
                if (currentNode!!.whiteSpace == 0 && currentNode!!.rootSize == 0) currentNode = node
            }
    ...
    fun getColourIndex(colour: Any?): Int = if (colour === Barcode.WHITE) getWhiteIndex() else getBlackIndex()
