...
class UniqueList<E> private constructor(
    ...
    @Suppress("NON_FINAL_MEMBER_IN_FINAL_CLASS", "UNCHECKED_CAST")
    open fun setComparator(comparator: Comparator<in E>?) {
    ...
    open fun getCount(index: Int): Int = getEndIndex(index) - getSourceIndex(index)

    @Suppress("NON_FINAL_MEMBER_IN_FINAL_CLASS")
    open fun getCount(value: E): Int {
        val index = indexOf(value)
        return if (index == -1) 0 else getCount(index)
    }

    @Suppress("NON_FINAL_MEMBER_IN_FINAL_CLASS")
    open fun getAll(index: Int): MutableList<E> {
        val startIndex = getSourceIndex(index)
        val endIndex = getEndIndex(index)
    ...
    @Suppress("NON_FINAL_MEMBER_IN_FINAL_CLASS")
    open fun getAll(value: E): MutableList<E> {
        val index = indexOf(value)
