...
class MainStartup private constructor() {
    companion object {
        ...
        private suspend fun activateNewMaxFilmLength() = withContext(Dispatchers.Swing) {
            ...
            if (!alreadyActivated) {
                ...
                Main.splashScreen.ifPresent { it.isVisible = true }
            }
        }
        ...
        private suspend fun startGuiMode() = withContext(Dispatchers.Swing) {
            Main.splashScreen.ifPresent { it.update(UIProgressState.INIT_FX) }

            Main.splashScreen.ifPresent { it.update(UIProgressState.FILE_CLEANUP) }
            if (SystemUtils.IS_OS_MAC_OSX) {
            ...
            Main.splashScreen.ifPresent { it.update(UIProgressState.START_UI) }
