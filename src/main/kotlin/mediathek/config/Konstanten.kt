...
object Konstanten {
    ...
    const val NEW_SENDER_ACTIVATED_QUESTION_CONFIG_KEY = "newSendersActivated.fourteen.three"
    const val NEW_FILMLENGTH_ACTIVATED_QUESTION_CONFIG_KEY = "newFilmlengthActivated.fourteen.three"

    @JvmField
    val MVVERSION = Version(14, 6, 0)

    @JvmField
    val APPLICATION_TYPE = ApplicationType.NIGHTLY

    const val MACOS_OFFICIAL_APP = "OSX_OFFICIAL_APP"

    const val FORMAT_ZIP = ".zip"
    const val FORMAT_XZ = ".xz"

    const val FULL_FILM_LIST = "Filmliste-akt.xz"
    const val DIFF_FILM_LIST = "Filmliste-diff.xz"
    const val AUDIOTHEK_SEARCH_TIMEOUT_SECONDS = 45L

    @JvmField
    val ALTER_FILMLISTE_SEKUNDEN_FUER_AUTOUPDATE: Long = TimeUnit.SECONDS.convert(3, TimeUnit.HOURS)
    ...
    const val MAX_DOWNLOAD_RESTARTS: Byte = 2
    const val MAX_EXTERNAL_STARTS: Byte = 3

    /**
     * Default time for automatic continuation of existing downloads.
     */
    const val DOWNLOAD_CONTINUATION_DEFAULT_TIME: Byte = 60
    const val DOWNLOAD_ERROR_DISPLAY_DURATION: Byte = 60
    ...
    const val VERZEICHNIS_DOWNLOADS = PROGRAMMNAME
    const val VERZEICHNIS_EINSTELLUNGEN = ".mediathek3"
    const val JSON_DATEI_FILME = "filme.json"
    const val BOOKMARK_FILE = "bookmarks.json"
     ...
     */
    @JvmField
    val MIN_FILM_FILE_SIZE_KB = 256 * FileUtils.ONE_KB

    const val XML_START = "Mediathek"
