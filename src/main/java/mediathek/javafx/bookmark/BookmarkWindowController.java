...
public class BookmarkWindowController implements Initializable {
    ...
    private ObservableList<BookmarkData> observableList;
    ...
    private void setupTableView() {
        var eventList = Daten.getInstance().getListeBookmarkList().getEventList();
        eventList.addListEventListener((ListEventListener) listChanges -> System.out.println("EVENTLIST CHANGED"));
        var bookmarkConnector = GlazedLists.beanConnector(BookmarkData.class);
        var lst = new ObservableElementList<>(eventList, bookmarkConnector);
        ...
        observableList.addListener((ListChangeListener.Change<? extends BookmarkData> c) -> {
            System.out.println("FX LISTENER CALLED");
            while (c.next()) {
    ...
    private void updateDisplay() {
        lblCount.setText(String.format("Einträge: %d", observableList.size()));
