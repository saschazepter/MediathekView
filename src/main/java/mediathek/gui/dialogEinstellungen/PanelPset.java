...
public class PanelPset extends JPanel {
    ...
    private void setupPSetVisiblePanels() {
        ...
        if (jCheckBoxAlleEinstellungen.isSelected()) {
            jPanelPset.add(new PanelPsetLang(parentComponent, Daten.listePset), BorderLayout.CENTER);
        } else {
            jPanelPset.add(new PanelPsetKurz(Daten.listePset), BorderLayout.CENTER);
        }
        jPanelPset.updateUI();
    }
