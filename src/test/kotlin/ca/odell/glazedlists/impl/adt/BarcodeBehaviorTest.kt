...
internal class BarcodeBehaviorTest {
    @Test
    fun javaAbiRetainsPublicIsEmptyWithoutExtraPropertyGetter() {
        val barcode = Barcode()
        val isEmpty = Barcode::class.java.getMethod("isEmpty")

        assertEquals(Boolean::class.javaPrimitiveType, isEmpty.returnType)
        assertTrue(invokeBoolean(isEmpty, barcode))

        barcode.addWhite(0, 2)
        assertFalse(invokeBoolean(isEmpty, barcode))

        barcode.clear()
        assertTrue(invokeBoolean(isEmpty, barcode))
        assertTrue(Barcode::class.java.methods.none { it.name == "getIsEmptyProperty" })
    }

    @Test
    fun barcodeInternalJvmSurfaceKeepsConstructorsPrivateAndInternalMethodsSynthetic() {
        assertOnlyRealPrivateConstructors(BarcodeNode::class.java)
        assertOnlyRealPrivateConstructors(BarcodeIterator::class.java)
        val internalSuffix = '$' + "MediathekView"

        listOf(
            "getRootNode$internalSuffix",
            "setRootNode$internalSuffix",
            "treeSize$internalSuffix",
            "treeSizeChanged$internalSuffix",
        ).forEach { assertSyntheticMethod(Barcode::class.java, it) }

        listOf(
            "getParent$internalSuffix",
            "setParent$internalSuffix",
            "getLeft$internalSuffix",
            "setLeft$internalSuffix",
            "getRight$internalSuffix",
            "setRight$internalSuffix",
            "getWhiteSpace$internalSuffix",
            "setWhiteSpace$internalSuffix",
            "getRootSize$internalSuffix",
            "setRootSize$internalSuffix",
            "size$internalSuffix",
            "blackSize$internalSuffix",
            "whiteSize$internalSuffix",
            "insertBlack$internalSuffix",
            "insertBlackAtEnd$internalSuffix",
            "insertWhite$internalSuffix",
            "getWhiteIndex$internalSuffix",
            "getBlackIndex$internalSuffix",
            "getIndexByWhiteIndex$internalSuffix",
            "getIndexByBlackIndex$internalSuffix",
            "getWhiteSequenceIndex$internalSuffix",
            "getBlackBeforeWhite$internalSuffix",
            "findSequenceOfMinimumSize$internalSuffix",
            "set$internalSuffix",
            "setWhite$internalSuffix",
            "setBlack$internalSuffix",
            "remove$internalSuffix",
            "removeBaseCase$internalSuffix",
            "validate$internalSuffix",
        ).forEach { assertSyntheticMethod(BarcodeNode::class.java, it) }

        assertSyntheticMethod(companionClass(BarcodeNode::class.java), "create$internalSuffix")
        assertSyntheticMethod(companionClass(BarcodeIterator::class.java), "create$internalSuffix")
    }
    ...
    private fun companionClass(type: Class<*>): Class<*> = Class.forName(type.name + '$' + "Companion")

    private fun assertOnlyRealPrivateConstructors(type: Class<*>) {
        val realConstructors = type.declaredConstructors.filterNot { it.isSynthetic }
        assertTrue(realConstructors.isNotEmpty(), type.name)
        assertTrue(realConstructors.all { Modifier.isPrivate(it.modifiers) }, type.name)
    }

    private fun assertSyntheticMethod(type: Class<*>, name: String) {
        val methods = type.declaredMethods.filter { it.name == name }
        assertTrue(methods.isNotEmpty(), name)
        assertTrue(methods.all(Method::isSynthetic), name)
    }

    private fun invokeBoolean(method: Method, target: Any): Boolean = method.invoke(target) as Boolean
