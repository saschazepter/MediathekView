...
internal class GroupingListMultiMapBehaviorTest {
    ...
    @Test
    fun sameKeySourceUpdateRetainsTheJavaUpdateSilenceAndPartialViewState() {
        val source = BasicEventList<String>().apply { addAll(listOf("a1", "a2", "b1")) }
        val map = newMap(source)
        val originalA = map["a"]!!
        val originalB = map["b"]!!
        ...
        assertEquals(listOf("a", "a", "b"), map.keys.toList())
        assertNotSame(originalA, map["a"])
        assertEquals(listOf("a9", "a2"), map["a"])
        assertEquals(listOf("b1"), originalA)
        assertThrows(NullPointerException::class.java) { map["b"].toString() }
        assertThrows(NullPointerException::class.java) { originalB.toString() }
    }
    ...
    @Test
    fun entrySetValueAddsNewElementsBeforeRemovingEqualOldElements() {
        ...
        assertTrue(source.isEmpty())
        assertFalse(map.containsKey("a"))
        assertEquals(0, entry.value!!.size)
    }
