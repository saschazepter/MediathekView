...
internal class FunctionListMapBehaviorTest {
    ...
    fun nullKeysAndValuesAreStoredButNullEntryConstructionRetainsItsDiagnostic() {
        ...
        val failure = assertThrows(IllegalArgumentException::class.java) { map.entries.iterator().next() }
        assertEquals("value cannot be null", failure.message)

        assertFalse(map.keys.remove(null))
